...
class BackendTask():
  def __init__( self, task_type, task_func, 
                                 pre_exec_func=None,
                                 post_exec_func=None ):
  ...
  def run(self, task_graph, reference_id, params):
    ...
    self.task_info = task_graph[reference_id]
    self.unique_id = self.task_info['unique_id']
    ...
    self.notify_job_pick_up(self.job, self.unique_id, self.task_info)
  
    # Log the beginning of the task
    log = MqttLog(node_id, self.unique_id)
    log.event(self.task_type, 'started')
    task_start_time = redis_tools.get_redis_server_time()
    tic = time.perf_counter()
  
    # Add handled job to execution list
    self.notify_job_exec_start(self.job, self.unique_id, node_id, self.task_info)
  
    # Perform the actual task
    task_results = self.task_func(task_graph, reference_id, params, node_id=node_id)
  
    # Update job progress
    toc = time.clock()
    self.notify_job_exec_end(self.job, self.unique_id, node_id, self.task_info, tic, toc)
    ...
    if not 'last_job' in list(task_results.keys()):
      ...
      self.task_type_id  = "{}_{}".format(self.task_info['type'], self.task_info['order'])
      redis_done_count_key  = R_TASK_TYPE_DONE.format(self.unique_id, self.task_type_id)
      redis_total_count_key = R_TASK_TYPE_TOTAL.format(self.unique_id, self.task_type_id)
      ...
      meta_tools.add_exec_time_info(self.unique_id, "{}-{}".format(self.task_type_id, self.task_info['seq_id']), task_start_time, task_end_time)
      log.event(self.task_type, "finished")
      log.exec_time(self.task_type, task_start_time, task_end_time)
  
      # Log partial results
      log.results(task_results['output'], subtype=self.task_type, metas=metas)

      return { 'sequence_ID': self.task_info['seq_id'], 
               'metas' : metas, 
    ...
    log.results(task_results['result'], subtype=self.task_type)
